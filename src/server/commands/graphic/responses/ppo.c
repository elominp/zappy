...
** Last update Fri Jun 24 17:03:13 2016 querat_g
*/
...
ppo(t_cli *c, t_msg *msg)
{
  ...
  g_i = 0;
  id = target->sock.fd;
  g_i = snprintf(buff, 16, "ppo %d ", id);
  snprintf(buff + g_i, 64 - g_i, "%ld %ld %u\n",
	   target->x_pos, target->y_pos, to_graphical_dir(target->direction));
  enqueue_msg(c, buff);
  return (true);
}
